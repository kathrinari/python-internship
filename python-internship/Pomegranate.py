...
import random, sys, time, math, pygame, pickle 
from pygame.locals import *


### KB: Setting up the variables in the main scope
achievement1 = 0
achievement2 = 0  # set the variable to record achievement

### KB: Setting up variables as conditions for while-loops
running=True
introduction=True
replay=True 
menu=True
...
##################### UNDER THIS LINE IS THE WORK THAT LINGZHI LI IS RESPONSIBLE FOR ##################################
...
##################### UNDER THIS LINE IS THE WORK THAT XIYUAN GAO IS RESPONSIBLE FOR #################################

# set up the window
SCREEN_SIZE = (640, 480)
screen = pygame.display.set_mode(SCREEN_SIZE, 0, 32)
# pictures
game_background = pygame.image.load("game_background.png").convert()  # background image
mouse_cursor = pygame.image.load("warrior.png").convert_alpha()  # mouse cursor
background_win = pygame.image.load("background_win.png").convert()  # background image
background_lose = pygame.image.load("background_lose.png").convert()

font = pygame.font.SysFont("arial", 32)  # font
...
text_surface_wrong = font.render("Oops! That's not right.", False, (255, 0, 0))  # lose_words
text_surface_reminder = font.render("Click to go back to menu.", False, (255, 0, 0))

text_surface_question2 = font.render("Which one is the Accusative form?", False, (255, 128, 0))  # question1
...
text_surface_final2 = font.render("You have not enough pomegranates to pass here!", False,
                                  (255, 0, 0))  # final words lose

# sound effects
effect_sound = pygame.mixer.Sound("effect_sound.wav")
...
def main():
    ...
    SMALLFONT = pygame.font.Font('freesansbold.ttf', 23)

    # load the image files
    L_WARRIOR_IMG = pygame.image.load('warrior.png')
    R_WARRIOR_IMG = pygame.transform.flip(L_WARRIOR_IMG, True, False)
    POMEGRANATE_TREE = pygame.image.load('tree_pom.png')
    TREE_1 = pygame.image.load('tree1.png')
    TREE_2 = pygame.image.load('tree2.png')
...
def runGame():
    ...
    while running:  # main game loop
        ...
        if not gameOverMode:
            ...
            if moveRight and playerObj['x'] != 620:
                playerObj['x'] += 5
                POSITION_X.append(int(playerObj['x']))
            if moveUp and playerObj['y'] != 0:
                playerObj['y'] -= 5
                POSITION_Y.append(int(playerObj['y']))
            if moveDown and playerObj['y'] != 460:
                playerObj['y'] += 5
                POSITION_X.append(int(playerObj['x']))
			...
			### KB: Changed the pixels to be more precise 
            if (playerObj['x'] >= 480 and playerObj['x'] <= 600) and (playerObj['y'] >= 80 and playerObj['y'] <= 180):
...
##################### ABOVE THIS LINE IS WORK THAT LINGZHI LI IS RESPONSIBLE FOR ##################################


##################### UNDER THIS LINE IS THE WORK THAT MENGYUAN XIAO IS RESPONSIBLE FOR ##################################
...
background_introduction = pygame.image.load("background_introduction.jpg").convert()
introduction_text = pygame.image.load("introduction_text.png").convert()
replay_text = pygame.image.load("replay_text.png").convert()
background_menu = pygame.image.load("background_menu.jpg").convert()
background_achievement = pygame.image.load("background_achievement.jpg").convert()

achievement_pomegranate = pygame.image.load("achievement_pomegranate.png").convert_alpha()


# handle_quit()
def quitmenu():
    global introduction, replay, menu, menu_achievement, running, question1, question2, final
	
    introduction=False
    replay=False
    menu=False 
    menu_achievement=False
    running=False
    question1=False 
    question2=False 
    final=False
...
# background music
def menu_music():
    background_music_ogg = "Hangout.ogg"
...
### KB: new function for replaying		
def pomegranate_replay():
    global replay
    while replay: 
        quitfunction()
        gameDisplay.blit(background_introduction, (0, 0))
        gameDisplay.blit(replay_text, (60, 130))
        pomegranate_text(50, "Introduction", HALF_WINWIDTH, QUARTILE_WINHEIGHT)
...
##################### ABOVE THIS LINE IS WORK THAT MENGYUAN XIAO IS RESPONSIBLE FOR ##################################
...
def question1():
    global question1, achievement1
    achievement1=0
    while question1:  # the game loop
        # exit option
        exit()
...
def question2():
    global question2, achievement2
    achievement2=0
    while question2:  # the game loop
        # exit option
        exit()
        # the background and texts for question1
        screen.blit(game_background, (0, 0))
    ...
    while final:
        ...
        for event in pygame.event.get():
            if event.type == pygame.MOUSEMOTION:
                ...
                pomegranate_replay() 				### KB: go to replay menu

        pygame.display.update()

##################### ABOVE THIS LINE IS WORK THAT XIYUAN GAO IS RESPONSIBLE FOR ##################################

def inventory(): 
'''KB: add pomegranates to hero's inventory'''
    f=open("hero.dat", "rb+")
    hero=pickle.load(f)  
	
    achievement=get() 
    for element in range(achievement): 
        hero.inventory_append("pomegranate") 
    hero.show_inventory()
    f.close()
